...
x2 <- x[x != med]
# Если каких-то значений не осталось, завершаем выполнение
if(length(x2) == 0) {
  stop("Все значения равны медиане. Невозможно выполнить медианный критерий.")
}
# Формируем последовательность знаков для оставшихся значений:
s <- ifelse(x2 > med, "+", "-")
...
# ======================================================================
# Разбиваем ряд на две группы. Если ряд нечётный, группа 2 получит на один элемент больше.
n <- length(x)
half <- floor(n / 2)
group1 <- x[1:half]
group2 <- x[(half + 1):n]
...
# ======================================================================
# Если гипотеза H0 принимается, ставим "OK", если отвергается – "*"
